...
export class GamesetComponent implements OnInit {
  ...
  onClick = (pipati: string) => {
    if (!this.isFinish) {
      ...
      let moveSelected = Object.values(MOVESELECT).filter(
        (res) => res === pipati
      );
  ...
  moveHand = setInterval(() => {
    ...
    this.movePlayer1 = `assets/myicons/${
      MOVEMENTS[this.countInterval].name
    }.png`;
    this.movePlayer2 = `assets/myicons/${
      MOVEMENTS[this.countInterval].name
    }.png`;
  ...
  checkWinner = (player1: string, player2: string) => {
    ...
    let lostWithPlayer1 = MOVEMENTS.filter((res) => res.name === player1)[0]
      .lostWith;
    player2 === player1 ? (this.alert = "TIED!") : false;
    player2 === lostWithPlayer1 ? (this.alert = "LOSE!") : false;
    player2 !== lostWithPlayer1 && player2 !== player1
      ? (this.alert = "WIN!")
      : false;
